...
function LoginScreen() {
    ...
    const res = await fetch("http://44.242.86.79/api/auth/login", {
  ...
  async function getAccount(id, authToken){
      const account = await fetch(`http://44.242.86.79/api/users/${id}`, {
        headers: {
  ...
  const submit = async () => {
    ...
    const friendRequestsRes = await fetch(`http://44.242.86.79/api/users/invites/${id}`, {
      headers: {
        "Authorization": `Bearer ${authToken}`,
   ...
   // dispatch(setCurrentConversation(bodyAcc.conversations[0]))
    dispatch(login())
    const sentInvitesRes = await fetch(`http://44.242.86.79/api/invite/sent/${id}`, {
      headers: {
        "Authorization": `Bearer ${authToken}`,
        "Content-Type": "application/json"
      }
    ...
    const receivedInvitesRes = await fetch(`http://44.242.86.79/api/invite/user/${id}`, {
      headers: {
        "Authorization": `Bearer ${authToken}`,
        "Content-Type": "application/json"
      }
